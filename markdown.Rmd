...
I begin the project by gathering the data from the website, with RVest package, into single columns, then wrangling and cleaning each one of them, and putting all of the information in a single dataframe. I then run a visual exploratory analysis to detect trends or patterns among the cryptocurrencies price variations in the data frame. Then, I make a feature selection process, which might include more data wrangling. The data set will then be ready for linear regression, that helps predicting the token’s prices and so, it’s stability. This might be complemented by the application of an arima model. 
